...
class convNet(object):
    ...
    def build_model(self):
        ...
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.logits, self.batch_labels))


        op_list.append(optimize) #
        with tf.control_dependency(op_list):
    ...
    def train(self):
        ...
        for step, (data_in, label_in) in enumerate(self.data_iteration(self.train_data, True)):
            _, loss, preds = self.sess.run([self.op, self.loss, self.preds],
                            feed_dict = {
                                    self.input_data: data_in,
                                    self.labels: label_in
                            }
        ...
        return total_loss, accuracy(total_preds, total_label)
    ...
    def test(self):
        ...
        for step, (data_in, label_in) in enumerate(self.data_iteration(self.test_data, False)):
    ...
    def run(self, task_name):
        ...
        test_loss, test_acc = self.test()
        print("Task: %d, Test loss: %.3f, Test Acc: %.3f" % (task_name, test_loss, test_acc))    
    ...
    def data_iteration(self, data, is_train = True):
        ...
        for l in xrange(batch_len):
            ...
            batch_labels = np.zeros((self.batch_size, 1), np.int32)
            ...
            batch_labels = (np.arange(num_labels) == labels[:, None]).astype(np.float32)
